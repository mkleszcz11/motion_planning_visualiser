...
class RandomWalkBiasedAlgorithm(Algorithm):
    ...
    def step(self):
        ...
        if random.random() < BIAS and self.map.goal:
            #print("Biasing towards goal") # Removed print for cleaner output
    ...
    def reconstruct_path(self):
        ...
        # The logic here is slightly different since RandomWalk doesn't guarantee
        # a node *exactly* at the goal.  We find the closest node.
        nearest_node = self.get_nearest_node((self.map.goal.x, self.map.goal.y))

        if nearest_node is None: # This might happen.
            return
